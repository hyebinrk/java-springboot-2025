...
public class Board {
    ...
    // ERD 1:N를 만드는 법
    @OneToMany(mappedBy = "board", cascade = CascadeType.REMOVE) // Reply 테이블과 1:N 관계. Board게시글이 삭제되면 그와 연결된 REply 댓글들도
                                                                 // 함께 삭제됨
    private List<Reply> replyList; // 댓글을 여러개 저장할 수 있도록 컬렉션(List)형태로 필드를 선언
